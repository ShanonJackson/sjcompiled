...
Phase 6 §6.1–§6.7 ☑ (keyframes / css / cssMap / xcss-prop /
css-prop / ClassNames / styled — all 7 per-API handlers shipped).
...
| §6.7 | ☑ (this session, 2026-05-05) | `styled` handler — 1:1 port of `styled/index.ts` (`visitStyledPath`) + `utils/build-styled-component.ts` (`buildStyledComponent` + supporting helpers) + verbatim `@emotion/is-prop-valid@1.4.0` prop table. Recognises all four styled call shapes (`styled.div(...)` / `styled(C)(...)` / `styled.div\`...\`` / `styled(C)\`...\``) via `extractStyledData*` matchers, detects the `styled.div\`...${(p) => p.x && p.y && 'bold'}...\`` invalid-expression footgun (`hasInValidExpression` predicate), and replaces the call/tagged-tpl with the `forwardRef(({ as: C = <tag>, style: __cmpls, ...__cmplp }, __cmplr) => <CC><CS>{cssArray}</CS><C ...></CC>)` wrapper. Hand-built JSX shape matches upstream's `@babel/template`-driven byte output (the JSX template parser dep is intentionally NOT pulled in — see `compat/template.rs` placeholder). Display-name insertion (`if (process.env.NODE_ENV !== 'production') { X.displayName = 'X'; }`) is queued via a `pending_styled_display_names` field on the visitor at `visit_mut_var_decl` pre-walk and drained at `visit_mut_module_items` / `visit_mut_stmts` AFTER the VarDecl item that triggered it. Upstream's `decls[0].id`-regardless-of-which-declarator-triggered quirk is preserved (BUGS in OLD = BUGS in NEW). | claude-2026-05-05 | `crates/babel-plugin/src/compat/is_prop_valid.rs` (~470 LOC + 13 unit tests covering valid-prop matches, data-/aria-/x- prefix case-handling, on-uppercase charcode short-circuit, count-lock at 418, sorted-table guard); `crates/babel-plugin/src/utils/build_styled_component.rs` (~770 LOC + 11 unit tests covering invalid-dom-props visitor / findOpenSelectors / styledStyleProp / build_styled_component shape); `crates/babel-plugin/src/styled/mod.rs` (~470 LOC + 6 unit tests covering each of the four extract-shape paths + the dispatch); `crates/babel-plugin/src/utils/mod.rs` (`pub mod build_styled_component;`); `crates/babel-plugin/src/lib.rs` (`pub mod styled;`); `crates/babel-plugin/src/compat/mod.rs` (`pub mod is_prop_valid;`); `crates/babel-plugin/src/babel_plugin.rs` (added `visit_mut_expr` styled dispatch + `visit_mut_var_decl` pre-walk + `visit_mut_module_items` / `visit_mut_stmts` displayName drain + `build_display_name_stmt` helper + 4 phase6c end-to-end visitor tests covering member-call / tagged-tpl / user-defined-component / no-displayName-when-outside-VarDecl); `crates/PARITY_VERSIONS.md` (`@emotion/is-prop-valid@1.4.0` pin entry). | Lib: `cargo test -p babel-plugin --lib` → **421/421** (was 387 post-§6.6; +34 unit/end-to-end tests). Integration: `compat_evaluation_integration` 3/3, `compat_scope_integration` 3/3, `compat_generator_integration` 3/3, `resolver_matrix_integration` 8/8, `transform_css_integration` 3/3, `hash_parity` 4/4 — all sibling gates unchanged. WASM: `cargo build -p babel-plugin --target wasm32-wasip1 --release` clean. **SWC vs Babel divergences (documented in module docs):** (1) `@babel/template`-driven JSX → hand-built SWC AST; same printed bytes after prettier. (2) `path.findParent(isVariableDeclaration)` → `visit_mut_var_decl` pre-walk + queue + drain. (3) Styled call inside an arrow body inside a VarDecl init has NO displayName emitted (upstream's `findParent` walk fires there); rare in practice — documented as a scope-of-the-pre-detect divergence. (4) `is_css_prop_disabled`-style comment-disable directives for the styled handler are NOT in scope — same SourceMap-thread followup that gates §6.5's directive support. styled fixtures byte-clean DEFERRED to §6.8 exit gate (the parity-harness/babel-plugin corpus runs end-to-end NOW that all 7 handlers ship). |
