...
pub fn lookup_name(input: &str) -> Option<RgbaColor> {
    ...
    crate::parse::parse(&crate::parse::ColordInput::Str(hex_target.to_string())).map(|(rgba, _)| rgba)
}

/// `toName()` reverse lookup — closest=false path.
pub fn rgba_to_name(rgba: crate::types::RgbaColor) -> Option<&'static str> {
    ...
    let rounded = crate::helpers::round_rgba(rgba);
    let hex = format!(
        "#{}{}{}",
        crate::helpers::hex_pair_str(rounded.r as u32),
        crate::helpers::hex_pair_str(rounded.g as u32),
        crate::helpers::hex_pair_str(rounded.b as u32),
    );
...
pub fn closest_name(rgba: crate::types::RgbaColor) -> &'static str {
    let mut best = "black";
    let mut best_dist = f64::INFINITY;
    for (name, hex) in NAME_TO_HEX.iter() {
        let candidate = match crate::parse::parse(&crate::parse::ColordInput::Str(hex.to_string())) {
            Some((c, _)) => c,
            None => continue,
        };
