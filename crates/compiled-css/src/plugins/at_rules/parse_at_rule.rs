//! Port of `packages/css/src/plugins/at-rules/parse-at-rule.ts`.
//!
...
/// `parseAtRule(params)` upstream. Returns parsed breakpoints in
...
pub fn parse_at_rule(params: &str) -> Vec<ParsedAtRule> {
...
mod tests {
    ...
    fn parses_single_min_width() {
        let r = parse_at_rule("(min-width: 200px)");
        assert_eq!(r.len(), 1);
    ...
    fn parses_max_width_to_le() {
        let r = parse_at_rule("(max-width: 400px)");
        assert_eq!(r.len(), 1);
    ...
    fn parses_range_syntax() {
        let r = parse_at_rule("(width <= 200px)");
        assert_eq!(r.len(), 1);
        assert_eq!(r[0].property, Property::Width);
        assert_eq!(r[0].comparison_operator, ComparisonOperator::Le);
    }
    ...
    fn parses_reversed_range_syntax_normalised() {
        ...
        let r = parse_at_rule("(200px >= width)");
        assert_eq!(r.len(), 1);
        assert_eq!(r[0].comparison_operator, ComparisonOperator::Le);
    }
    ...
    fn parses_combined_query_in_source_order() {
        let r = parse_at_rule("(min-width: 100px) and (max-width: 500px)");
        assert_eq!(r.len(), 2);
    ...
    fn drops_unknown_units() {
        ...
        let r = parse_at_rule("(min-width: 100vw)");
        assert!(r.is_empty(), "got: {:?}", r);
    }

    #[test]
    fn print_query_returns_no_breakpoints() {
        let r = parse_at_rule("print");
        assert!(r.is_empty());
    }
}
