...
use super::constants::angle_factor;
use super::helpers::{clamp_rgba, clamp_hsla, hsla_to_rgba, round};
...
pub fn parse(input: &ColordInput) -> Option<(RgbaColor, &'static str)> {
    match input {
        ...
        ColordInput::Hsva(c) => Some((super::helpers::hsva_to_rgba(*c), "hsv")),
...
fn parse_string(s: &str) -> Option<(RgbaColor, &'static str)> {
    ...
    if let Some(c) = super::names::lookup_name(s) { return Some((c, "name")); }
    None
}
...
fn parse_object(o: &ObjectInput) -> Option<(RgbaColor, &'static str)> {
    ...
    if o.h.is_some() && o.s.is_some() && o.v.is_some() {
        let a = o.a.unwrap_or(1.0);
        let hsva = super::helpers::clamp_hsva(HsvaColor { h: o.h.unwrap(), s: o.s.unwrap(), v: o.v.unwrap(), a });
        return Some((super::helpers::hsva_to_rgba(hsva), "hsv"));
    }
    None
}
