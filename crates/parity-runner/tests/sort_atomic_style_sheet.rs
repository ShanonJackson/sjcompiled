//! Integration test: `sort-atomic-style-sheet` parity, JS vs Rust.
//!
//! Exercises the orchestrator that combines sort-shorthand-declarations,
//! sort-pseudo-selectors, and parse-media-query / sort-at-rules.
...
#[test]
fn sort_atomic_style_sheet_parity() {
    ...
    for (label, css) in &entries {
        let js_resp = js.run(Stage::SortAtomicStyleSheet, css).expect("JS bridge call");
        if !js_resp.ok {
            failures.push(format!("[{label}] JS error: {}", js_resp.error));
            continue;
        }
        let rs_out = match rust_run_stage(Stage::SortAtomicStyleSheet, css) {
            Ok(s) => s,
            Err(e) => {
