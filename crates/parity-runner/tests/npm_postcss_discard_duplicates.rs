//! Integration test: npm `postcss-discard-duplicates@6` parity, JS vs Rust.
...
#[test]
fn npm_postcss_discard_duplicates_parity() {
    ...
    let corpus = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("corpus")
        .join("npm-postcss-discard-duplicates");
    ...
    for (label, css) in &entries {
        let js_resp = js
            .run(Stage::NpmPostcssDiscardDuplicates, css)
            .expect("JS bridge call");
        if !js_resp.ok {
            failures.push(format!("[{label}] JS error: {}", js_resp.error));
            continue;
        }
        let rs_out = match rust_run_stage(Stage::NpmPostcssDiscardDuplicates, css) {
            Ok(s) => s,
            Err(e) => {
