//! Integration test: `atomicify-rules` parity, JS vs Rust.
//!
//! This is the **CRITICAL** parity test (Phase 4d) — the hash output
//! becomes the class names in production. A single byte of drift here
//! rotates every Compiled-generated class name in the consuming
//! codebase.
...
fn atomicify_rules_parity() {
    ...
    for (label, css) in &entries {
        let js_resp = js.run(Stage::AtomicifyRules, css).expect("JS bridge call");
        if !js_resp.ok {
            failures.push(format!("[{label}] JS error: {}", js_resp.error));
            continue;
        }
        let rs_out = match rust_run_stage(Stage::AtomicifyRules, css) {
            Ok(s) => s,
            Err(e) => {
