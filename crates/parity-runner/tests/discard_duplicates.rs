//! Integration test: LOCAL `discard-duplicates` parity, JS vs Rust.
//!
//! Distinct from `postcss-discard-duplicates@6.0.0` (the v6 used by
//! `sort.ts`) — see Anomaly #9 in `PARITY_VERSIONS.md`.
//!
...
fn discard_duplicates_parity() {
    ...
    for (label, css) in &entries {
        let js_resp = js.run(Stage::DiscardDuplicates, css).expect("JS bridge call");
        if !js_resp.ok {
            failures.push(format!("[{label}] JS error: {}", js_resp.error));
            continue;
        }
        let rs_out = match rust_run_stage(Stage::DiscardDuplicates, css) {
            Ok(s) => s,
            Err(e) => {
