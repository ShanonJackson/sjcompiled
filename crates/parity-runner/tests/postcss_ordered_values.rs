//! Integration test: `postcss-ordered-values@5.1.3` parity, JS vs Rust.
...
#[test]
fn postcss_ordered_values_parity() {
    ...
    for (label, css) in &entries {
        let js_resp = js.run(Stage::PostcssOrderedValues, css).expect("JS bridge call");
        if !js_resp.ok {
            failures.push(format!("[{label}] JS error: {}", js_resp.error));
            continue;
        }
        let rs_out = match rust_run_stage(Stage::PostcssOrderedValues, css) {
            Ok(s) => s,
            Err(e) => {
