//! Integration test: `parent-orphaned-pseudos` parity, JS vs Rust.
//!
...
#[test]
fn parent_orphaned_pseudos_parity() {
    ...
    for (label, css) in &entries {
        let js_resp = js.run(Stage::ParentOrphanedPseudos, css).expect("JS bridge call");
        if !js_resp.ok {
            failures.push(format!("[{label}] JS error: {}", js_resp.error));
            continue;
        }
        let rs_out = match rust_run_stage(Stage::ParentOrphanedPseudos, css) {
            Ok(s) => s,
            Err(e) => {
