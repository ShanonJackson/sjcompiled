//! Integration test: `increase-specificity` parity, JS vs Rust.
//!
...
#[test]
fn increase_specificity_parity() {
    ...
    for (label, css) in &entries {
        let js_resp = js.run(Stage::IncreaseSpecificity, css).expect("JS bridge call");
        if !js_resp.ok {
            failures.push(format!("[{label}] JS error: {}", js_resp.error));
            continue;
        }
        let rs_out = match rust_run_stage(Stage::IncreaseSpecificity, css) {
            Ok(s) => s,
            Err(e) => {
