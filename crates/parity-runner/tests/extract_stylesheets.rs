//! Integration test: `extract-stylesheets` parity, JS vs Rust.
//!
//! The plugin is read-only — it pushes per-child `node.toString()` strings
//! into a callback. The harness joins them with U+001E (record separator)
//! to ride the single-line bridge protocol; both sides must produce
//! byte-identical joined output, which means each sheet is byte-identical
//! AND the document order matches.
//!
...
fn extract_stylesheets_parity() {
    ...
    for (label, css) in &entries {
        let js_resp = js.run(Stage::ExtractStylesheets, css).expect("JS bridge call");
        if !js_resp.ok {
            failures.push(format!("[{label}] JS error: {}", js_resp.error));
            continue;
        }
        let rs_out = match rust_run_stage(Stage::ExtractStylesheets, css) {
            Ok(s) => s,
            Err(e) => {
