//! Integration test: `normalize-current-color` parity, JS vs Rust.
...
#[test]
fn normalize_current_color_parity() {
    ...
    for (label, css) in &entries {
        let js_resp = js.run(Stage::NormalizeCurrentColor, css).expect("JS bridge call");
        if !js_resp.ok {
            failures.push(format!("[{label}] JS error: {}", js_resp.error));
            continue;
        }
        let rs_out = match rust_run_stage(Stage::NormalizeCurrentColor, css) {
            Ok(s) => s,
            Err(e) => {
