//! Integration test: `expand-shorthands` parity, JS vs Rust.
...
fn expand_shorthands_parity() {
    ...
    for (label, css) in &entries {
        let js_resp = js.run(Stage::ExpandShorthands, css).expect("JS bridge call");
        if !js_resp.ok {
            failures.push(format!("[{label}] JS error: {}", js_resp.error));
            continue;
        }
        let rs_out = match rust_run_stage(Stage::ExpandShorthands, css) {
            Ok(s) => s,
            Err(e) => {
