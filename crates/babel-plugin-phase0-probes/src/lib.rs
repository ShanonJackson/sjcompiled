...
fn run_wasi_io(call_scratch: &str) {
    // SWC's wasmer backend mounts host cwd at /cwd. The host passes
    // us a /cwd-prefixed path; we simply write/read at that path.
    ...
    eprintln!("[probe wasi-io] write {:?} => {:?}", probe_path, write_res);
    let read_back = write_res.as_ref().ok().and_then(|_| fs::read(&probe_path).ok());
    eprintln!(
        "[probe wasi-io] read  {:?} => len={:?}",
        probe_path,
        read_back.as_ref().map(|b| b.len())
    );
