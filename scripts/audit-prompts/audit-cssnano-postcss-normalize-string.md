# Re-audit: `postcss-normalize-string@5.1.0` (no drift) → `cssnano-postcss-normalize-string`

> **Crate mapping:** the npm package `postcss-normalize-string` is ported as the Rust crate `cssnano-postcss-normalize-string` (under `crates/cssnano-postcss-normalize-string/`). The crate name diverges from the npm name on purpose — see PARITY_VERSIONS.md for the convention. **All file/path references in the rest of this prompt point at `cssnano-postcss-normalize-string`, not at a hypothetical `postcss-normalize-string` crate.**
...
## Specific to `postcss-normalize-string`

`postcss-normalize-string@5.1.0` is **NOT** drifted between the
...
Default `preferredQuote: "double"`. Touches every quoted string value.

## Source locations

- **AFM-pinned source (5.1.0)**: `node_modules/.bun/postcss-normalize-string@5.1.0/node_modules/postcss-normalize-string/`
- **Rust port**: `crates/cssnano-postcss-normalize-string/`
...
Walk every file in `node_modules/.bun/postcss-normalize-string@5.1.0/node_modules/postcss-normalize-string/src/`.
...
crates/target/debug/parity-runner --stage postcss-normalize-string --corpus crates/parity-runner/corpus/postcss-normalize-string --determinism
```
...
Write a concise audit document at `crates/_vendor/POSTCSS_NORMALIZE_STRING_5.1.0_REAUDIT.md` containing:
